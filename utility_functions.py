...
def calAccuracyForEachSession(df, targetItemList):
    ...
    for i in range(len(targetItemList)):
        ...
        recom_ser = pd.Series(data=[i for i in range(len(tempList))], index=tempList)
        next_item = int(targetItemList[i])
        if next_item in recom_ser:
             rank = recom_ser.index.get_loc( next_item ) + 1
             mrr.append(1.0 / rank)
        else:
             mrr.append(0.0)
